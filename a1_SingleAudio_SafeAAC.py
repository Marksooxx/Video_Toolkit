# -*- coding: utf-8 -*-
"""
音频替换脚本 - 安全AAC版本 (解决 AAC 编码导致的 ducking 问题)

与原版 a1_SingleAudio_ReplaceOriginal.py 的区别:
- 添加 -af volume=1.0 音频过滤器，强制禁用任何自动增益控制
- 使用更高比特率 (320kbps) 减少压缩损失
- 保持 MP4 输出格式，完全兼容原流程

原理说明:
- AAC 编码器在处理某些音频片段时可能自动调整增益，导致 ducking 效果
- 通过 volume=1.0 过滤器，强制保持原始音量不变
- 更高的比特率可以减少编码器对动态范围的压缩

使用方法:
1. 将此脚本放在包含 MP4 视频的目录
2. 在同目录下创建 ~audio 文件夹，放入同名的 WAV 音频文件
3. 运行脚本，输出将保存在 ~mix 文件夹，格式为 MP4
"""
import os
import sys
import subprocess
...
# --- 配置区 ---
SCRIPT_NAME = "Mix_Audio_Video_SafeAAC"
SCRIPT_VERSION = "1.0.0"
MAX_WORKERS = os.cpu_count() or 4
INPUT_AUDIO_DIR = "~audio"  # 音频输入文件夹
OUTPUT_MIX_DIR = "~mix"     # 输出文件夹
DEFAULT_ENCODING = 'utf-8'

# --- 音频编码设置 ---
# AAC 比特率 (更高 = 更好质量，推荐 256k-320k)
AAC_BITRATE = "320k"
# 输出格式保持 MP4
OUTPUT_EXT = ".mp4"
...
def process_video_task(video_file):
    """
    处理单个视频文件的任务 (替换音频，使用安全AAC编码)。
    输出时长为视频和音频中较长者。
    返回 'success', 'skipped', or 'failed'。
    """
    task_id = os.path.basename(video_file)
    logging.info(f"--- [タスク {task_id}] 処理開始 (モード: 音声置換 - SafeAAC) ---")
    video_name = os.path.splitext(task_id)[0]
    
    # 音频输入路径
    audio_file = os.path.abspath(os.path.join(INPUT_AUDIO_DIR, video_name + ".wav"))
    # 输出文件路径 (保持 MP4 格式)
    output_file = os.path.abspath(os.path.join(OUTPUT_MIX_DIR, video_name + OUTPUT_EXT))
    ...
    try:
        ...
        logging.debug(f"[タスク {task_id}] 音声ファイルを検索: {audio_file}")

        if not os.path.exists(audio_file):
            logging.warning(f"[タスク {task_id}] スキップ - 一致する音声ファイルが見つかりません: {audio_file}")
            status_to_return = 'skipped'
            return status_to_return

        logging.info(f"[タスク {task_id}] 音声が見つかりました。メディア情報の取得を開始します...")

        video_duration_str = run_ffprobe(abs_video_file, 'format=duration')
        audio_duration_str = run_ffprobe(audio_file, 'format=duration')

        if video_duration_str is None or audio_duration_str is None:
            logging.error(f"[タスク {task_id}] ビデオまたは音声の長さを取得できませんでした。失敗としてマークします。")
            return status_to_return

        try:
            video_duration = float(video_duration_str)
            audio_duration = float(audio_duration_str)
            logging.info(f"[タスク {task_id}] ビデオの長さ: {video_duration:.3f}秒, 音声の長さ: {audio_duration:.3f}秒")
        except ValueError:
            logging.error(f"[タスク {task_id}] ビデオまたは音声の長さの解析に失敗しました (値: '{video_duration_str}', '{audio_duration_str}')。失敗としてマークします。")
            return status_to_return
        ...
        if audio_duration > video_duration + duration_tolerance:
            ...
            logging.info(f"[タスク {task_id}] 音声はビデオより約 {duration_diff:.3f}秒長いため、ビデオを約 {audio_duration:.3f}秒に延長するために黒画面を生成します。")
            ...
            logging.info(f"[タスク {task_id}] ビデオは正常に約 {audio_duration:.3f}秒に延長されました。")
        else:
            logging.info(f"[タスク {task_id}] 音声の長さはビデオより長くありません ({audio_duration:.3f}秒 <= {video_duration:.3f}秒)。出力はビデオの長さ ({video_duration:.3f}秒) になります。")

        # --- Final Merge Command (安全AAC模式) ---
        logging.info(f"[タスク {task_id}] ビデオと新しい音声をマージ開始 (SafeAAC モード)...")
        
        # 安全AAC编码设置:
        # -af volume=1.0: 强制保持原始音量，禁用任何自动增益
        # -b:a 320k: 使用更高比特率减少压缩损失
        # -aac_coder twoloop: 使用更精确的 AAC 编码算法
        merge_cmd = ['-i', video_input_for_merge,  # 原始或扩展后的视频
                     '-i', audio_file,              # 新音频 (WAV)
                     '-map', '0:v:0',               # 视频来自输入 0
                     '-map', '1:a:0',               # 音频来自输入 1
                     '-c:v', 'copy',                # 复制视频流（不重编码）
                     '-af', 'volume=1.0',           # 【关键】强制保持原始音量
                     '-c:a', 'aac',                 # 使用 AAC 编码
                     '-b:a', AAC_BITRATE,           # 高比特率
                     '-aac_coder', 'twoloop',       # 使用更精确的编码算法
                     output_file]
        
        logging.debug(f"[タスク {task_id}] SafeAAC マージコマンド: {' '.join(['ffmpeg'] + merge_cmd)}")

        if run_ffmpeg_command(merge_cmd):
            final_duration_str = run_ffprobe(output_file, 'format=duration')
            logging.info(f"[タスク {task_id}] 音声の置換に成功 (SafeAAC): {output_file} (最終的な長さ: {final_duration_str}秒)")
...
def main():
    ...
    logging.info(f"モード: 音声置換 (SafeAAC - volume=1.0 + {AAC_BITRATE})。")
    logging.info(f"出力形式: MP4 (AAC音声)")
    ...
    logging.info(f"スキップされたタスク数 (音声なし): {skipped_tasks}")
    ...
    if skipped_tasks > 0: logging.warning("スキップされたタスクがあります (対応する音声ファイルが見つからなかったため)。")
