...
def process_video_task(video_file):
    ...
    logging.info(f"--- [タスク {task_id}] 処理開始 (モード: 音声置換) ---")
    ...
    try:
        ...
        logging.debug(f"[タスク {task_id}] 音声ファイルを検索: {audio_file}") # Path now relative to CWD
        ...
        # --- Final Merge Command (Replace Mode) ---
        logging.info(f"[タスク {task_id}] ビデオと新しい音声をマージ開始 (置換モード、長さは長い方に合わせる)...")
        merge_cmd = ['-i', video_input_for_merge, # Original or Extended Video
                     '-i', audio_file,          # New Audio
        ...
        if run_ffmpeg_command(merge_cmd):
            ...
            logging.info(f"[タスク {task_id}] 音声の置換に成功し、出力しました: {output_file} (最終的な長さ: {final_duration_str}秒)")
...
def main():
    ...
    logging.info("モード: 音声置換。出力の長さは、元のビデオと新しい音声のいずれか長い方になります。")
