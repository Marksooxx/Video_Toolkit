# -*- coding: utf-8 -*-
"""
音频替换脚本 - 无损版本 (解决 AAC 编码导致的 ducking 问题)

与原版 a1_SingleAudio_ReplaceOriginal.py 的区别:
- 使用 FLAC 无损音频编码，避免 AAC 编码器可能导致的音量突然下降问题
- 输出格式为 MKV (因为 MP4 不支持 FLAC 音频)
- 完全保留原始音频质量，无任何压缩损失

使用方法:
1. 将此脚本放在包含 MP4 视频的目录
2. 在同目录下创建 ~audio 文件夹，放入同名的 WAV 音频文件
3. 运行脚本，输出将保存在 ~mix 文件夹，格式为 MKV
"""
import os
import sys
import subprocess
...
# --- 配置区 ---
SCRIPT_NAME = "Mix_Audio_Video_Lossless"
...
# --- 音频编码设置 ---
# 使用 FLAC 无损编码，完全避免 AAC 编码可能导致的 ducking 问题
# 输出容器必须是 MKV（因为 MP4 不支持 FLAC）
OUTPUT_EXT = ".mkv"
...
def process_video_task(video_file):
    """
    处理单个视频文件的任务 (替换音频，使用无损编码)。
    输出时长为视频和音频中较长者。
    返回 'success', 'skipped', or 'failed'。
    """
    task_id = os.path.basename(video_file)
    logging.info(f"--- [タスク {task_id}] 処理開始 (モード: 音声置換 - 無損) ---")
    ...
    try:
        ...
        # --- Final Merge Command (无损模式 - FLAC 编码) ---
        logging.info(f"[タスク {task_id}] ビデオと新しい音声をマージ開始 (無損モード: FLAC)...")
        
        # 使用 FLAC 无损音频编码
        # -c:a flac: 使用 FLAC 编码器，完全无损
        # -compression_level 8: FLAC 压缩级别 (0-12)，越高压缩率越好但速度越慢
        ...
        logging.debug(f"[タスク {task_id}] 無損マージコマンド: {' '.join(['ffmpeg'] + merge_cmd)}")

        if run_ffmpeg_command(merge_cmd):
            final_duration_str = run_ffprobe(output_file, 'format=duration')
            logging.info(f"[タスク {task_id}] 音声の置換に成功 (無損出力): {output_file} (最終的な長さ: {final_duration_str}秒)")
...
def main():
    ...
    logging.info("モード: 音声置換 (無損 FLAC)。出力の長さは、元のビデオと新しい音声のいずれか長い方になります。")
    logging.info(f"出力形式: MKV (FLAC音声)")
